...
struct LouvainOptions {
  ...
  LouvainOptions(int repeat=1, size_t accumulatorCapacity=0, T resolution=1, T tolerance=1e-2, T passTolerance=0, T tolerenceDeclineFactor=10, int maxIterations=500, int maxPasses=500) :
  repeat(repeat), accumulatorCapacity(accumulatorCapacity), resolution(resolution), tolerance(tolerance), passTolerance(passTolerance), tolerenceDeclineFactor(tolerenceDeclineFactor), maxIterations(maxIterations), maxPasses(maxPasses) {}
};
 ...
 * @param vcom community each vertex belongs to
 */
template <bool SELF=false, bool CAP=false, class K, class V>
void louvainScanCommunity(vector<K>& vcs, vector<V>& vcout, K u, K v, V w, const vector<K>& vcom) {
  if (!SELF && u==v) return;
  K c = vcom[v];
  K k = !CAP? c : c % vcout.size();
  if (!vcout[k]) vcs.push_back(c);
  vcout[k] += w;
}
 ...
 * @param vcom community each vertex belongs to
 */
template <bool SELF=false, bool CAP=false, class G, class K, class V>
void louvainScanCommunities(vector<K>& vcs, vector<V>& vcout, const G& x, K u, const vector<K>& vcom) {
  x.forEachEdge(u, [&](auto v, auto w) { louvainScanCommunity<SELF, CAP>(vcs, vcout, u, v, w, vcom); });
}
...
auto louvainChooseCommunity(const G& x, K u, const vector<K>& vcom, const vector<V>& vtot, const vector<V>& ctot, const vector<K>& vcs, const vector<V>& vcout, V M, V R) {
  ...
  V emax = V();
  K d = vcom[u];
  K l = !CAP? d : d % vcout.size();
  for (K c : vcs) {
    K k = !CAP? c : c % vcout.size();
    if (!SELF && c==d) continue;
    V e = deltaModularity(vcout[k], vcout[l], vtot[u], ctot[c], ctot[d], M, R);
 ...
 */
template <bool CAP=false, class G, class K, class V, class FA, class FP>
...
template <bool CAP=false, class G, class K, class V, class FA>
inline int louvainMove(vector<K>& vcom, vector<V>& ctot, vector<K>& vcs, vector<V>& vcout, const G& x, const vector<V>& vtot, V M, V R, V E, int L, FA fa) {
  auto fp = [](auto u) {};
  return louvainMove<CAP>(vcom, ctot, vcs, vcout, x, vtot, M, R, E, L, fa, fp);
}
...
inline int louvainMove(vector<K>& vcom, vector<V>& ctot, vector<K>& vcs, vector<V>& vcout, const G& x, const vector<V>& vtot, V M, V R, V E, int L) {
  ...
  return louvainMove<CAP>(vcom, ctot, vcs, vcout, x, vtot, M, R, E, L, fa);
}
